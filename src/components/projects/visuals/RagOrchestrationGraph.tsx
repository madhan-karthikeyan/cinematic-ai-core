      ...
      >
        ...
        <g>
          <ellipse
            ...
            rx={130}
            ry={170}
            fill="color-mix(in oklab, var(--accent-violet) 6%, transparent)"
            stroke="color-mix(in oklab, var(--accent-violet) 18%, transparent)"
            strokeDasharray="2 5"
          />
          {DOCS.map((d) => (
            <motion.rect
              ...
              fill={
                trace
                  ? "color-mix(in oklab, var(--accent-cyan) 60%, transparent)"
        ...
        {/* reranker → context */}
        <Edge
          d={`M ${RERANKER.x + 16} ${RERANKER.y} L ${CONTEXT.x - 16} ${CONTEXT.y}`}
          color="color-mix(in oklab, var(--accent-warm) 26%, transparent)"
          pulseDur={2.4}
        />
        {/* context → LLM */}
        <Edge
          d={`M ${CONTEXT.x + 16} ${CONTEXT.y} L ${LLM.x - 16} ${LLM.y}`}
          color="color-mix(in oklab, var(--accent-warm) 32%, transparent)"
          pulseDur={2}
        />
